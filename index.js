...
async function verifyPayment(txHash) {
  try {
    ...
    const wallet = process.env.OPTIMISM_USDC_WALLET.toLowerCase();
...
async function getLiveRates() {
  try {
    const [ecbRes, cgRes] = await Promise.all([
      ...
      fetch("https://api.coingecko.com/api/v3/simple/price?ids=bitcoin,ethereum,solana,optimism-2,binancecoin,matic-network,chainlink,near,cosmos,ripple,algorand,celo&vs_currencies=usd"),
    ]);
    ...
    const crypto = {
      ...
      SOL: cg["solana"]?.usd, AVAX: cg["optimism-2"]?.usd,
...
<html><head><title>Bercy Optimism Pay</title>
<style>
...
<h1>🔺 Bercy Optimism Pay</h1>
<p>World's first <b>x402 + AC2</b> cross-border payment layer on Optimism Mainnet.</p>
<div>
  <span class="badge">⚡ x402 on Optimism</span>
...
<hr>
<p>🔺 Optimism C-Chain | Chain 10 | x402 + AC2<br>
...
app.get("/api/health", async (req, res) => {
  try {
    ...
    res.json({ status: "ok", service: "Bercy Optimism Pay", network: "Optimism Mainnet", protocols: ["x402", "AC2"], totalCorridors: 95 });
  }
});
...
app.post("/api/authorize", async (req, res) => {
  ...
  const approval_id = "ac2_op_" + Date.now() + "_" + Math.random().toString(36).slice(2, 8);
  res.json({
    approved: true,
    approval_id,
    from, to, amount,
    ...
    message: "AC2: Human approval granted. Proceed with bercy_pay_optimism.",
    timestamp: new Date().toISOString()
  });
});
...
app.post("/api/orchestrate", async (req, res) => {
  ...
  if (!payment) return res.status(402).json({
    ...
    pay_to: process.env.OPTIMISM_USDC_WALLET,
    message: "Send $0.10 USDC on Optimism C-Chain, include tx hash as X-PAYMENT header"
  });
  ...
  const tag = "op_" + Date.now().toString(36);
  res.json({
    success: true,
...
app.listen(PORT, () => console.log("Bercy Optimism Pay running on port " + PORT));
